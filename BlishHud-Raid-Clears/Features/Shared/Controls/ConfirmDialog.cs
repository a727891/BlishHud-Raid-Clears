...
public class ConfirmDialog : Container
{
    ...
    public ConfirmDialog(string title, string message, ButtonDefinition[] buttons = null)
    {
        ...
        this._backgroundImage = Service.Textures!.SettingWindowBackground;
    ...
    private void SelectButton(int direction = 0)
    {
        ...
        {
            this._buttons[this.SelectedButtonIndex].Button.BackgroundColor = Service.Settings.StrikeSettings.Style.Color.Cleared.Value.HexToXnaColor();
        }
        else
        {
    ...
    {
        return await this.ShowDialog(TimeSpan.FromMinutes(1), cancellationToken);
    }
    ...
    public override void RecalculateLayout()
    {
        ...
        this._titleRect = new Rectangle(this._confirmRect.X + textPadding, this._confirmRect.Y + this._confirmRect.Height / 5,
            this._confirmRect.Width - textPadding * 2, (int)Math.Ceiling(_titleFont.MeasureString(this._parsedTitle).Height));

        this._messageRect = new Rectangle(this._confirmRect.X + textPadding, this._titleRect.Bottom + 15,
